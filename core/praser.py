...
def parse(args):
    ...
    if args.input_img is not None:
        opt["input_img"] = args.input_img
    if args.input_mask is not None:
        opt["input_mask"] = args.input_mask
    if args.keep_temp is not None:
        opt["keep_temp"] = args.keep_temp
    if args.nodata_value is not None:
        try:
            opt["nodata_value"] = float(args.nodata_value)
        except:
            try:
                opt["nodata_value"] = int(args.nodata_value)
            except:
                opt["nodata_value"] = None


    if args.tile_overlap is not None:
        try:
            opt["tile_overlap"] = int(args.tile_overlap)
        except:
            opt["tile_overlap"] = 0
