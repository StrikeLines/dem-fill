...
# Changed Here! DEM Normalize is replaced
...
class DEMNormalize(object):
    ...
    def __call__(self, height_meter):
        self.min_val, self.max_val = torch.min(height_meter), torch.max(height_meter)
        diff = self.max_val - self.min_val
        if diff == 0:
            return torch.zeros(height_meter.shape)
        height = (height_meter - self.min_val) / diff
        height = 2 * height - 1
        return torch.clamp(height, -1, 1)

    def denormalize(self, norm_tensor):
        height = (norm_tensor + 1) / 2
        return height * (self.max_val - self.min_val) + self.min_val
