...
if self.opt.get('benchmark_inference', False):
    ...
    def set_input(self, data):
        ...
        #Changed here! added get min_val and max_val and scaleFactor
        self.min_val = data.get('min_val')  # Add
        self.max_val = data.get('max_val')  # Add
        self.scaleFactor = data.get('scaleFactor') # Add
        # print("HERE IS OUT SCALEFACTOR self.scaleFactor,",self.scaleFactor, data.get('scaleFactor'))
    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                # print(f"[DEBUG] Output stats for {self.path}:")
                # print(f"  Shape: {self.output.shape}")
                # print(f"  Min: {self.output.min().item():.4f}, Max: {self.output.max().item():.4f}")
                # print(f"  Mean: {self.output.mean().item():.4f}")
                ...
                for key, value in self.get_current_visuals(phase='test').items():
                    self.writer.add_images(key, value)
                #changed here! Added min_val and max_val
                # print(f"at save_images -> {self.min_val, self.max_val}")
                self.writer.save_images(self.save_current_results(),min_val=self.min_val,max_val=self.max_val, scaleFactor = self.scaleFactor)
